...
function testFitSampModPVTeval_MgPvTange2012(testCase)
    ...
    verifyTrue(testCase,false,...
        'testFitSampModPVTeval_MgPvTange2012 not yet implemented');
    %verifyTrue(testCase,true,'Must be able to initialize PVTdata without error');
end
function testGetEos_NeDewaele2008(testCase)
    PTVDewaele2008 = [...
          4.53  298 54.7897;...
          7.55  298 49.2613;...
          9.38  298 46.7299;...
        ...
        205.20  298 21.4989;...
        208.80  298 21.3970];
    Pne = PTVDewaele2008(:,1);
    Tne = PTVDewaele2008(:,2);
    Vne = PTVDewaele2008(:,3);

    eosNeMod = getEos_NeDewaele2008();
    [Pmod,Pderivsmod,KTmod,Cvmod,gammod,thmExpmod] = ...
        evalPressEos([],eosNeMod,Vne,Tne);
    Presid = Pne-Pmod;
    verifyTrue(testCase, abs(mean(Presid)/std(Presid)) < 1,...
        'The Neon Eos must match the data from Dewaele 2008.');
end
